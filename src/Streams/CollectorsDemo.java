...
public class CollectorsDemo {
    public static void main(String[] args) {
        ...
        List<String> fruitsC = Arrays.asList("Apple", "Banana", "Orange", "Apple", "Mango", "Banana", "Grapes", "Apple");
        final var stringIntegerMap = fruitsC.stream().collect(Collectors.toMap(k -> k, v -> 1, Integer::sum));
        System.out.println(stringIntegerMap);

        final var collect1 = fruitsC.stream().collect(Collectors.groupingBy(String::length));
        System.out.println(collect1);

        final var collect2 = fruitsC.stream().collect(Collectors.partitioningBy(x -> x.length() == 6, Collectors.groupingBy(x -> x, Collectors.counting())));
        System.out.println(collect2);
        List<List<Integer>> numbers1 = List.of(
                List.of(1, 2, 3),
                List.of(4, 5),
                List.of(6, 7, 8)
        );
        final var list = numbers1.stream()
                .flatMap(List::stream)
                .toList();
